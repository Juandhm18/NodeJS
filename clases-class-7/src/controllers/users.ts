...
import { getAllUsers as getUserService } from '../services/users.service.ts'
import { IUser } from '../interfaces/book.interface.ts'

const __filename = fileURLToPath(import.meta.url)
const __dirname = path.dirname(__filename)
const pipelineAsync = promisify(pipeline)

const USERS_FILE = path.join(__dirname, '../models/users.jsonl')

const getAllUsers = async (req:Request, res:Response): Promise<void> =>{
    let statusCode:HttpErrorStatus = 500;
    try{
        const all = await getUserService();
        res.send(all);
    }catch(err){
